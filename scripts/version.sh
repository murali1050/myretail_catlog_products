...
if     [[ -z $(which envsubst 2> /dev/null) ]] \
    ...
    die "You must install required dependencies [envsubst, git, jq, yaml]."
fi
...
tag_build() {
    ...
    if [ -z ${TAGTYPE+x} ]; then
        die "Missing required input(s): [TAG_TYPE]"
    fi
    case "${TAGTYPE}" in
        alpha|beta|dev|rc)
            ...
            ;;
        *)
            die "TAG_TYPE '${TAGTYPE}' is not a buildable tag [alpha, beta, dev, rc]"
            ;;
    esac
    ...
    local git_describe=$(git describe --tags --always --match "${VERSION}-${TAGTYPE}*")
    local build=0
    local prebuild=0
    ...
    if [[ $(git describe --tags --exact-match "${git_commit}" 2>/dev/null) == "${VERSION}-${TAGTYPE}.${prebuild}" ]]; then
        echo "Commit already tagged; no tag applied"
        exit 0
    fi

    echo "Adding tag: ${VERSION}-${TAGTYPE}.${build}"
    git tag "${VERSION}-${TAGTYPE}.${build}"
}
...
rc_version() {
    ...
    git config --local --bool gitflow.allowdirty true

    # create the release candidate branch
    git flow release start "${VERSION}-${TAGTYPE}.${build}"

    git config --local --unset gitflow.allowdirty

    # assume the release will actually be promoted by removing pretag value
    release_version
...
promote() {
    ...
    if [[ $(git rev-parse --abbrev-ref HEAD) != "master" ]]; then
        die "Not on 'master' branch; no tag applied"
    fi
    ...
    # Skip trying to add tag again.
    local git_commit=$(git rev-parse --short HEAD)
    if [[ $(git describe --tags --exact-match "${git_commit}" 2>/dev/null) == "${VERSION}" ]]; then
        echo "Commit already tagged; no tag applied"
        exit 0
    fi

    CURRENT_PRE_TAG=$(git describe --tags --always)

    echo "Promoting ${CURRENT_PRE_TAG} to ${VERSION}"
    git tag -a -m "Release ${VERSION}" "${VERSION}"
}

rc_to_dev() {
    if [[ $(git rev-parse --abbrev-ref HEAD) != "develop" ]]; then
        die "Not on 'develop' branch"
    fi
    ...
    git commit "${PROJECT_FILE}" -m 'Task: Restore dev mode after creating Release Candidate'
}
...
dev_version() {
    case "${TAGTYPE}" in
        alpha|beta|dev|rc)
            ...
            ;;
        *)
            die "Unexpected TAG_TYPE '${TAGTYPE}'; Valid values: [alpha, beta, dev, rc]"
            ;;
    esac
...
case "$MODE" in
    ...
    rel)
        ...
        ;;
    *)
      die "Available execution modes: [build, dev, promote, rc, rctodev, rel]"
esac
